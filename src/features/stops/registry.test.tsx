...
import * as table from "./houseTableMetrics";
import { STOPS } from "./registry";
import * as life from "./sharedLifeMetrics";
  ...
  it("renders each section through its own stop, in the section's own element", () => {
    for (const s of site.scene) {
      ...
      if (s.id === "house-churches") {
        expect(drawing(table.TABLE_VIEW_W, table.TABLE_VIEW_H)).not.toBeNull();
        expect(drawing(life.LIFE_VIEW_W, life.LIFE_VIEW_H)).toBeNull();
      }
      if (s.id === "about") {
        expect(drawing(life.LIFE_VIEW_W, life.LIFE_VIEW_H)).not.toBeNull();
        expect(drawing(table.TABLE_VIEW_W, table.TABLE_VIEW_H)).toBeNull();
      }
      unmount();
    }
  });
