...
export type PentecostParallaxProps = Partial<Omit<SceneOptions, "reliefMax">> & {
  ...
  tier?: Tier;
  /** the scene sections the camera paces itself against, by ref (scroll/sectionRefs.ts) */
  sections: { readonly current: readonly HTMLElement[] };
  className?: string;
};

/**
 * The scene as a component: mounts createParallaxScene() against its own
 * canvas (and the front one), forwards the live options, and disposes. The
 * loading progress and the ready signal go to the app store; a debug build
 * hangs the built scene on the page's seam for tools/shots/cdp-rects.mjs.
 * Under StrictMode the first mount is disposed whole before the second
 * builds, so each canvas only ever has one live renderer.
 */
export default function PentecostParallax({
  ...
  const once = useRef({ tier, rays, embers, frontCanvas, sections }); // read once, at mount
  // live props, so tweaking them never rebuilds the scene: this effect runs
  // before the mount effect below, so the scene builds with the first values
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const { tier, rays, embers, frontCanvas, sections } = once.current;
    ...
    handle.current = createParallaxScene(
      ...
      { tier, rays, embers, reducedMotion, sections: () => sections.current, onProgress: setProgress, onReady: markReady, debug },
      options.current,
    );
    return () => {
